...
const Forum = () => {
  ...
  const detectSuspiciousActivity = (topic) => {
    const commentCount = topic.comments_count || topic.comments?.length || 0;
    // Use individual comment data if available, otherwise estimate from count
    const uniqueUsers = Array.isArray(topic.comments) && topic.comments.length > 0
      ? new Set(topic.comments.map(c => c.user_id || c.user?.id).filter(Boolean))
      : new Set([Math.min(Math.max(1, Math.floor(commentCount * 0.7)), commentCount)]);
  ...
  const fetchTopics = async (reset = false) => {
    try {
      ...
      if (reset) {
        ...
        setTopics(prev => [...prev, ...(Array.isArray(data.data) ? data.data : [])]);
        setPage(prev => prev + 1);
      }
    ...
    try {
      ...
      if (reportTarget.type === 'topic') {
        ...
        if (selectedTopicForModal) {
          const updatedComments = Array.isArray(selectedTopicForModal.comments) ? selectedTopicForModal.comments.map(comment => 
          ...
          <View style={styles.rightPanel}>
            ...
            {Array.isArray(topics) && topics
              ...
              .map(topic => {
                ...
                const uniqueUsers = Array.isArray(topic.comments) && topic.comments.length > 0
                  ? new Set(topic.comments.map(c => c.user_id || c.user?.id).filter(Boolean))
                  : new Set([Math.min(Math.max(1, Math.floor((topic.comments_count || 0) * 0.7)), topic.comments_count || 0)]);
                return (
                  <TouchableOpacity
                    key={topic.id}
                    style={[
                    ...
                    <View style={styles.recentTopicRow}>
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.topicScore}>
                          Score: {score} ({topic.comments_count || 0} comments, {uniqueUsers.size} users)
                          {isSuspicious && (
